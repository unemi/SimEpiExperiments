...
plot () {
local nm=$1
local bv=`echo $1 | awk -F_ '{printf "%s_%s_BVcn\n",$1,$2}'`
local yr=""
if [ $# -gt 2 ]; then
  nm=$1_`echo $3 | cut -c2-`
  yr="set yrange [:$3]"
...
plot '$bv.csv' lc rgb "#666666", for [i=2:(${nf}+1)] '$1.csv' using 1:i
EOF
}
plotA () {
local nm=$outfn
...
mkBVcn () {
if [ -d $1 ]; then
awk -F, '$1<='$vdStep'{printf "%.4f\t%.6f\n",$1/16,($2+$3)/'$pop'}'\
  $1/indexes_1.csv > in_$2_BVcn.csv
awk -F, '$1<='$vd'{printf "%d\t%.8f\n",$1,$2/'$pop'}'\
  $1/daily_1.csv > tp_$2_BVcn.csv
fi
}
#
...
LANG=ja_JP.UTF-8
#
# if [ ! -f in_fair_BVcn.csv ]; then
mkBVcn MyResult_00 fair
mkBVcn MyResult_05 worse
#
for d in MyResult_??; do
cd $d
nx=`echo $d | awk -F_ '{printf "%s_%02d_'$gf'\n",($2<5)?"fair":"worse",2^($2%5)}'`
n=0;s=0
...
awk -F, '$1<='$vd'{printf "%d\t%.8f\n",$1,$2/'$pop'}' daily_1.csv > ../tp_${nx}_bv.csv
rm -f XX.csv
for ((x=$vd;x<=$n;x++)); do echo $x >> XX.csv; done
for f in daily_*.csv; do
  awk -F, '$1>='$vd'{printf "\t%.8f\n",$2/'$pop'}\
  END{for(i=NR;i<='$n';i++) print "\t0"}' $f > X$f
  lam XX.csv X$f > X2.csv
  rm XX.csv X$f; mv X2.csv XX.csv
done
mv XX.csv ../tp_$nx.csv
#
...
echo "\"day\"" > $outfn.csv
for ((x=$vd;x<$tl;x++)); do
  for y in {0..3}; do echo $y | awk '{printf "%.2f\n",$1/4+'$x'}' >> $outfn.csv; done
done
echo $tl.00 >> $outfn.csv
for ff in in_${cc}_[0-9]*_${gf}.csv; do
  vp=`echo $ff | cut -d_ -f3`
  awk 'BEGIN{printf "\t\"%.1f%%\"\n",'$vp'/10;z='$vd'}\
{if($1==z){s=0;for(i=2;i<=NF;i++)s+=$i;printf "\t%.6f\n",s/(NF-1);z+=.25}}\
END{for(x=z;x<='$tl';x+=.25)print "\t0"}' $ff > X$vp.csv
done
...
n=0
echo "\"day\"" > $outfn.csv
for ((x=$vd;x<=$tl;x++)); do echo $x >> $outfn.csv; done
for ff in tp_${cc}_[0-9]*_${gf}.csv; do
  vp=`echo $ff | cut -d_ -f3`
  awk 'BEGIN{printf "\t\"%.1f%%\"\n",'$vp'/10;z='$vd'-1}\
