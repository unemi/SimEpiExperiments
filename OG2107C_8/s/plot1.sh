...
if [ $mk = "E" ];then rd3=201; ed3=236
elif [ $mk = "F" ]; then rd3=201; ed3=250
...
LANG=ja_JP.UTF-8
#
dst="/Users/unemi/Program/SimEpidemic/Documents/Figures/OC_8"
#
makePlot () {
echo plot $1$2.svg
...
if ("$1" eq "IN") {
 ...
 set yrange [0:.3]; set y2label "gatherings frequency (%)"; set y2range [:7]  }
else {
 set terminal svg size 720 300; set key right bottom; set ylabel "weekly test positive (%)"
 set yrange [0:.014]; set y2label "1st dose vaccinated (%)\ngatherings frequency (‰)" }
