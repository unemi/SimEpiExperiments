...
foreach f (indexes_*.csv)
  awk -F, '$1>0{x+=$2+$3;n++;if($1%'$skp'==0){printf "\t%.2f\n",x/n/'$pop'.;x=0;n=0}}' $f > X$f
  echo "$n "`tail -1 $f | cut -d, -f1` |\
   awk '{for(i=$2/'$skp';i<$1/'$skp';i++) print "\t0"}' >> X$f
end
lam X*.csv | awk '{printf "%.2f%s\n",NR*'$skp'/16.,$0}' > infected
rm X*.csv
...
set key left title "infection rate"
set style data lines
set xlabel "days"
set ylabel "infected (%)"
set xrange [:365]
plot for [i=${nf}:0:-1] 'infected' using 1:i+2 title sprintf("%d%%",40+5*i)\
