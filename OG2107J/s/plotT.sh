...
declare -a arr
LANG=C
local tspan=$(((tl+1-$3)/5))
for ((x=0;x<5;x++)); do local d=$((x*tspan+tspan/2-1+$3))
...
#
echo plot TP$1-$2-$3.svg
gnuplot > TP$1-$2-$3.svg <<EOF
set terminal svg size 640 300
set key right
set style data lines
set xrange [$3:${tl}]
do for [d in "46 116 150 199 242"] {
set arrow from d,graph 0 to d,graph 1 nohead lc rgb "#884400" }
if ("$1" eq "date") { set xtics ($xmarks) }
if ("$2" eq "log") { set yrange $yr; set logscale y }
...
plot for [i=2:$nf] 'testPositive.csv' using 1:i \
 ...
 'gatFreq.csv' using 1:2 axis x1y2 title "gat.freq." lc rgb "#666666",\
 '/Users/unemi/Research/SimEpidemicPJ/内閣府PJ/統計データ/nhk_pref_weekly/13東京都.csv'\
  using (\$1 - 338):(\$2 / 139600.0) title "Tokyo" lc rgb "#008800" lw 2
EOF
}
makePlot date linear 1
makePlot day linear 1
makePlot day log 1
if [ $tl -gt 150 ]; then
  makePlot date linear $((tl-100))
  makePlot day linear $((tl-100))
  makePlot day log $((tl-100))
fi
#
open .
# open -a Safari TPAverageLog.svg