...
foreach f (indexes_*.csv)
  ...
  if (-f XX) then
    lam XX X$f > XXX; mv XXX XX; rm X$f
  else
    mv X$f XX
  endif
end
...
set ylabel "infected (%)"
set xrange [:200]
set label "${pp}%" at graph .025,.85
plot for [i=2:(${nf}+1)] 'infected' using 1:i
EOF
lam $pdir/infecAve infecAve > $pdir/infecAve2
rm infecAve
popd > /dev/null
...
set ylabel "infected (%)"
set xrange [:200]
plot for [i=0:${nf}] 'infected' using 1:i+2 title sprintf("%.1f%%",i*0.1)\
