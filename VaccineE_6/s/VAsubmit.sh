...
nGFs=3
tl=258 # August 31
dir=`pwd | awk -F/ '{if($(NF-1) ~ /^V_[0-9]_[0-9][0-9][0-9]_[0-9][0-9]$/\
...
st=`echo $dir | awk -F_ '{printf "E095_%s\n",$3}'`
pr=`echo $dir | awk -F_ '{printf "%d\n",$5}'`
vp=`echo $dir | awk -F_ '{print $2}'`
va=`echo $dir | awk -F_ '{printf "%d\n",$4}'`
...
if [ $ld = 106 ]; then nj=$nGFs; else nj=$((nn*nGFs)); fi
