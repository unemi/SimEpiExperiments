...
careless=`echo $dirName | awk -F_ '{print $2}'`
delay=`echo $dirName | awk -F_ '{print $4}'`
mk=`echo $dirName | awk -F_ '{printf "%s_%s_%s",$1,$2,$4}'`
gfClmn=`echo $dirName | awk -F_ '{print ($2==0)?2:($2==1)?4:3}'`
...
#
LANG=C
rd2x=`date -j -v+${rd2}d 121601002020 "+%b %e" | sed 's/  / /'`
tspan=$(((tl-1)/10))
...
#
nj=0
for jobDir in ../E_${careless}_*_${delay}; do
nj=$((nj+1))
...
if ("$1" ne "TP") { plot\
 ...
 '  .csv' using 1:2 axes x1y2 title "vaccinated" lc rgb "#880066" }
EOF
}
#
makePlot IN
