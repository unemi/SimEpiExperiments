#! /bin/bash
tl=258 # August 31
dir=`pwd | awk -F/ '{if($(NF-1) ~ /^P.*_[0-9][0-9][0-9]_[0-9][0-9][0-9]$/\
 && $NF ~ /^[0-9][0-9]$/)printf "%s_%s\n",$(NF-1),$NF;else print 1}'`
if [ $dir = 1 ]; then echo "This script must run on directory P?_999_999/99."; exit; fi
st=`echo $dir | awk -F_ '{printf "R_%s_%s\n",$2,$3}'`
gf=`echo $dir | awk -F_ '{printf "%d\n",$4}'`
vp=`echo $dir | awk -F_ '{print (length($1)==1)?0:substr($1,2,1)}'`
