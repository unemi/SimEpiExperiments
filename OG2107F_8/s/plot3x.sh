...
LANG=ja_JP.UTF-8
#
dst="/Users/unemi/Program/SimEpidemic/Documents/Figures/OF_8"
#
makePlot () {
echo plot $1$2.svg
gnuplot > $dst/${mk}_${bias}_$1$2x.svg <<EOF
if ("$1" eq "IN") {
 set terminal svg size 680 300; set key left; set ylabel "infected (%)"
 set yrange [0:.5]; set y2range [:10]; set y2label "gatherings frequency (%)" }
else {
 set terminal svg size 720 300; set key left; set ylabel "weekly test positive (%)"
 set yrange [0:.025]; set y2range [:100]
