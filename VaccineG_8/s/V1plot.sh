...
if [ $nd = 1 ]; then echo "This script must run from V_?_???_??."; exit; fi
prs=`echo $nd | awk -F_ '{print $4}'`
pr=`echo $prs | awk '{printf "%d\n",$1}'`
...
mm=`echo $x | awk '{print $1*5+10}'`
nx=`echo $mm | awk '{printf "x%.1f\n",$1/10}'`
n=0;s=0
...
awk '{printf "%.2f%s\n",(NR-1)*'$s'/'$spd'+'$cmd',$0}' XX.csv > infected.csv
rm XX.csv
...
#
if [ "$1" == "" ]; then
dst="/Users/unemi/Program/SimEpidemic/Documents/Figures/VPF_$popN/${vp}_${vd}_${prs}_$va"
